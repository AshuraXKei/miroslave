...
export default function Footer() {
  ...
  return (
    <footer className="bg-gray-900 dark:bg-black text-white relative">
      {/* Top border gradient — warm amber */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
              ...
              {["Главная", "Обо мне", "Навыки", "Опыт", "Проекты", "Контакты"].map(
                (item) => (
                  <button
                    ...
                    className="text-left text-sm text-gray-400 hover:text-amber-400 transition-colors py-1"
                  >
                    {item}
                  </button>
                )
              ...
              {socialLinks.map((link) => {
                ...
                return (
                  <a
                    ...
                    className="w-11 h-11 rounded-xl bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-amber-600 hover:text-white transition-all hover:-translate-y-0.5"
                  >
        ...
        <div className="mt-12 pt-8 border-t border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={scrollToTop}
            className="w-11 h-11 rounded-xl bg-gray-800 flex items-center justify-center text-gray-400 hover:bg-amber-600 hover:text-white transition-all"
            aria-label="Наверх"
          >
            <FaArrowUp />
          </motion.button>
